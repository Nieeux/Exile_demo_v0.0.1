    ...
    private void BuildNavMesh(bool Async)
    {
        ...
        for (int index = 0; index < Surfaces.Length; index++)
        {
            if(MarkupsPerSurface[index].Count == 0)
            {
                ...
                else if (Surfaces[index].collectObjects != CollectObjects.Children && ModifiersPerSurface[index].Count == 0)
                {
                ...
                {
                    if (((Surfaces[index].layerMask & (1 << ModifiersPerSurface[index][i].gameObject.layer)) == 1)
